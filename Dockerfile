...
ADD data-api.sh /usr/bin/data-api.sh
ADD _data-api.sh /usr/local/share/zsh/site-functions/_data-api.sh
ADD data-api.sh.bash-completion /etc/bash-completion.d/data-api.sh
RUN chmod 755 /usr/bin/data-api.sh
...
RUN echo '\n\
. /etc/bash_completion\n\
source /etc/bash-completion.d/data-api.sh\n\
' >> ~/.bashrc
...
\n\
$(tput setaf 3)DATA_API_HOST$(tput sgr0) - server URL, e.g. https://example.com:8080\n\
\n\
